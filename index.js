...
const handler = co.wrap(function *(req, res, next) {
    ...
    let inner_header = String(config.INNER_HEADER ? (req.header(config.INNER_HEADER) || ''): '');
    ...
    try {
        ...
        data.ip = req.ip;
        let header = {};
        if(config.INNER_HEADER) header[config.INNER_HEADER] = inner_header;
        ...
        if(!body.UserID) {
            let error = new Error('get userinfo from django error ==> body: ' + JSON.stringify(body).substring(0, 100));
            error.detail = body;
            throw error;
        }
